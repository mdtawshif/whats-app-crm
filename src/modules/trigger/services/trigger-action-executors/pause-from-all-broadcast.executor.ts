...
@Injectable()
export class PauseFromAllBroadcastActionExecutor extends BaseActionExecutor {
    constructor(
        ...
        this.logger.setContext(PauseFromAllBroadcastActionExecutor.name);
    }
    ...
    async executeAction(
        ...
        this.logger.info('Executing pause from all broadcasts', { queueId: cacheTriggerEventActionQueue.id });
        ...
        try {
            ...
            //             `${TriggerUtils.normalizeActionKey(cacheTriggerEventActionQueue.triggerEventType)}: Paused ${getContactDisplayName(parsedPayload?.contact as unknown as Contact) || 'Contact'} from all broadcasts`,
            //             result?.message,
            //             '/broadcasts',
            //             skipNotification
            //         );
            ...
            return this.handleError(cacheTriggerEventActionQueue, triggerEvent, `Failed to execute ${ActionKeys.PAUSE_FROM_ALL_BROADCAST}`, error, ActionKeys.PAUSE_FROM_ALL_BROADCAST);
        }
    }
}