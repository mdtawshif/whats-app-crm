import { BillingPackage, CyclePeriod, Prisma, TransactionType, User, UserPackage, UserPackageRenewHistoryStatus } from "@prisma/client";
...
@Injectable()
export class PackageRenewService {
  ...
  constructor(
    ...
    @InjectPinoLogger(PackageRenewService.name)
  ...
  /**
   * Send success email after package renew.
   */
  async sendPackageRenewSuccessMail(amount: number, user: User): Promise<boolean> {
    ...
    if (user?.email) {
      ...
      const subject = `Package Renew Successful - $${amount} charged`;
      const body = `
            <p>Dear ${fullName || "User"},</p>
            <p>We have successfully processed your package renew of <strong>$${amount.toFixed(2)}</strong>.</p>
 ...
 */
  ...
  async chargeFromCurrentCreditForce(userData: User, packageRenewUserCardInfoData: PackageRenewUserCardInfo, amount: number, userPackage: UserPackage, billingPackageData: BillingPackage): Promise<boolean> {
    ...
    const userTransactionData = await this.prisma.billingTransaction.create({
      data: {
        ...
        createdBy: packageRenewUserCardInfoData.userId,
        agencyId: agency.id,
        creditAmount: amount,
  ...
  async chargeFromCurrentCredit(userData: User, packageRenewUserCardInfoData: PackageRenewUserCardInfo, amount: number, userPackage: UserPackage, billingPackageData: BillingPackage): Promise<boolean> {
    ...
    const userTransactionData = await this.prisma.billingTransaction.create({
      data: {
        userId: userData.parentUserId ? userData.parentUserId : Number(packageRenewUserCardInfoData.userId),
        agencyId: agency.id,
        creditAmount: amount,
        billingPackageId: packageRenewUserCardInfoData.packageId,
        type: TransactionType.OUT,
  ...
  async handlePackageRenewProcess() {

    console.log('handlePackageRenewProcess started ########################');

    const packageRenewUsers: PackageRenewUserCardInfo[] =
    ...
    console.log("packageRenewUsers", packageRenewUsers);

    for (const user of packageRenewUsers) {
      ...
      if (userData.currentCredit.toNumber() >= amount) {
        const packageRenewChargeFromCurrentCredit = await this.chargeFromCurrentCredit(userData, user, amount, userPackage, billingPackageData);
